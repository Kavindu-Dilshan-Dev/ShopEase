...
public class CartActivity extends AppCompatActivity implements CartAdapter.CartListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        cartDb.cartDao().getAllItems().observe(this, items -> {
            ...
            binding.tvSubtotal.setText(String.format("LKR %.2f", subtotal));
            binding.tvDelivery.setText(String.format("LKR %.2f", delivery));
            binding.tvTotal.setText(String.format("LKR %.2f", subtotal + delivery));
    ...
    public void onQuantityChanged(CartItem item, int newQuantity) {
        ...
        Executors.newSingleThreadExecutor().execute(() -> cartDb.cartDao().update(item));
    }

    @Override
    public void onRemove(CartItem item) {
        Executors.newSingleThreadExecutor().execute(() -> cartDb.cartDao().delete(item));
    }
}